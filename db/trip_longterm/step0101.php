	...
	<div class="cont-trip">
		<div class="step-wrap">
			...
			<ul class="Sign-up stage">
				<li>
					<strong>여행종류</strong>
					<span class="choiceChk third">
						<input type="radio" id="trvTypeChk1" name="trip_type" value="3" checked>
						<label for="trvTypeChk1">장기</label>
					</span>
				...
				<li>
					...
					<div class="select-box">
						<select name="trip_purpose">
							<option value="1" <?=$_SESSION["travel_step"]["1"]["trip_purpose"]=="1"?"selected='selected'":""?>>유학/연수</option>
							<option value="2" <?=$_SESSION["travel_step"]["1"]["trip_purpose"]=="2"?"selected='selected'":""?>>주재원</option>
							<option value="3" <?=$_SESSION["travel_step"]["1"]["trip_purpose"]=="3"?"selected='selected'":""?>>업무/출장</option>
							<option value="4" <?=$_SESSION["travel_step"]["1"]["trip_purpose"]=="4"?"selected='selected'":""?>>워킹홀리데이</option>
						</select>
					</div>
				</li>
			</ul>
			...
			<div class="thtoknow">
				...
				<div class="konw-box">
					<ul>
						...
						<li class="type-dash">4. 해외여행 중 현지에서 사고 발생 시 24시간 한국어 안내가 지원되는 메리츠화재 우리말서비스 82-2-360-2407로 연락주시면 도움 받으실 수 있습니다. 수정 필요!!!!!!!!</li>
					</ul>
				</div>
			</div>
...
<?php 
require_once $SYS_ROOT_DIR."/travel/db/modal/restrict.php"; //보험인수 제한 국가 안내 btn-restr
?>
...
<script type="text/javascript">
	...
	$(document).on('change','input[name=trip_type]',function() {
			...
			if($('#start_date').val()) {
				$("#end_date").datepicker( "option", "maxDate", new Date(Date.parse(treemonthcal($('#start_date').val(), '12'))) );
			}
	...
	$(document).on('click','a[name=btnNext]',function() {
		...
		$.ajax({
			type : "POST",
			url : "/travel/db/common_longterm/process_step0101.php",
...
//				$("#end_date").datepicker( "option", "minDate", new Date(Date.parse(selectedDate)) );
				$("#end_date").datepicker( "option", "maxDate", new Date(Date.parse(treemonthcal(selectedDate, '12'))) );
...
//			minDate: today,
			...
			onClose: function( selectedDate ) {  
				...
				var date_term=dateDiff($("#start_date").val(),$("#end_date").val());
					...
					if (parseInt(date_term)>365) {
						alert('시작일로 부터 1년 까지만 가입이 가능합니다.');
					}
				}
			}
		});
