...
if ($tripType=="2") {
?>
								<a href="/travel/db/files/db.overseas.pdf" class="button lineBtn" target="_blank">해외여행보험약관 다운로드</a>
<?php
} else {
?>
...
<?php 
require_once $SYS_ROOT_DIR."/travel/db/modal/agreement.php"; //이용약관	btn-agre
require_once $SYS_ROOT_DIR."/travel/db/modal/insurance.php"; //보험약관	btn-insu
require_once $SYS_ROOT_DIR."/travel/db/modal/collective.php"; //개인정보 처리 및 단체가입규약 동의	btn-colle
require_once $SYS_ROOT_DIR."/travel/db/modal/marketing.php"; //마케팅 활용 동의	btn-mark
require_once $SYS_ROOT_DIR."/travel/db/modal/privacy.php"; //개인정보수집 및 이용	btn-priv

require_once $SYS_ROOT_DIR."/travel/db/modal/hospitalize.php"; //입원, 수술, 질병확진 btn-hosp
require_once $SYS_ROOT_DIR."/travel/db/modal/specific.php"; //특정질병 btn-spec
require_once $SYS_ROOT_DIR."/travel/db/modal/sports.php"; //위험한 레포츠 btn-sport
require_once $SYS_ROOT_DIR."/travel/db/modal/restrict.php"; //보험인수 제한 국가 안내 btn-restr
?>

<link rel="stylesheet" type="text/css" href="/travel/db/css/modal.css?v=<?=filemtime($_SERVER['DOCUMENT_ROOT']."/travel/db/css/modal.css")?>">
...
$(document).on('click','a[name=btnNext]',function() {
	...
	*/?>		
	$.ajax({
		type : "POST",
		url : "/travel/db/common/process_step04.php",
