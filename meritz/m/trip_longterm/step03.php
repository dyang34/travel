...
if(chk_step_session($pageStepNo)) {
	JsUtil::alertReplace("이전 가입하던 정보가 존재하지 않습니다.\\r\\n\\r\\다시 입력해 주십시오.","/travel/meritz/m/trip_longterm/step0101.php");
...
?>
			<div class="step03">
				<h3><?=$title?> (<?=$arrMember[$i]['gender']=="1"?"<strong class=\"man\">남</strong>":"<strong class=\"wom\">여</strong>"?>)</h3>
				<ul class="clearfix inb">
					...
					<li class="eng">
						...
						<input type="text" name="name_eng_last[]" class="input01 first" pattern="[A-Za-z]+" placeholder="성" value="<?=$_SESSION["travel_step"]["1"]["member"][$i]['name_eng_last']?>" oninput="this.value = this.value.replace(/[^A-Za-z.]/g, '')">
						<input type="text" name="name_eng_first[]" class="input01" pattern="[A-Za-z]+" placeholder="이름" value="<?=$_SESSION["travel_step"]["1"]["member"][$i]['name_eng_first']?>" oninput="this.value = this.value.replace(/[^A-Za-z\s.]/g, '')">
					</li>
					<li class="resident">
						<strong>주민등록번호(외국인등록번호)</strong>
						<input type="tel" class="input01 first" placeholder="앞 6자리" value="<?=substr($arrMember[$i]['birth'],2,6)?>" readonly>
						<span>-</span>
						<input type="password" class="input01" gender="<?=$arrMember[$i]['gender']?>" name="jumin2[]" placeholder="*******" maxlength="7" value="<?=$_SESSION["travel_step"]["1"]["member"][$i]['jumin2']?>">
					</li>
					<li class="last">
						<strong>영주권자 / 이중국적자
...
?>
					<li class="tel">
						...
						<span>-</span>
						<input type="number" class="input01" placeholder="0000" maxlength="4" name="hphone2" oninput="maxLengthCheck(this, 'hphone3')" value="<?=$_SESSION["travel_step"]["1"]["member"][$i]['hphone2']?>">
						<span>-</span>
						<input type="number" class="input01" placeholder="0000" maxlength="4" id="hphone3" name="hphone3" oninput="maxLengthCheck(this, 'email1')" value="<?=$_SESSION["travel_step"]["1"]["member"][$i]['hphone3']?>">
					</li>
					<li class="email">
						<strong>이메일 주소</strong>
						<input type="text" name="email1" class="input01" placeholder="hong" id="email1"placeholder="이메일" type="email" value="<?=$_SESSION["travel_step"]["1"]["member"][$i]['email1']?>">
						<span>@</span>
						<input type="text" name="email2" class="input01" placeholder="bis.com" value="<?=$_SESSION["travel_step"]["1"]["member"][$i]['email2']?>">
					</li>
<?php
	}
?>
...
$(document).on('click','a[name=btnNext]',function() {
	...
	$.ajax({
		type : "POST",
		url : "/travel/meritz/common_longterm/process_step03.php",
...
<?php
require_once $SYS_ROOT_DIR."/travel/meritz/m/modal/edit-info_longterm.php";
