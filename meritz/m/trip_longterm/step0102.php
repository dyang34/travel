			...
			<div class="step divPartner">
				...
				<li>
					...
					<div class="select-box">
						<select name="job">
							<option value="1" <?=$_SESSION["travel_step"]["1"]["job"]=="1"?"selected='selected'":""?>>사무직 </option>
							<option value="2" <?=$_SESSION["travel_step"]["1"]["job"]=="2"?"selected='selected'":""?>>학생 </option>
							<option value="3" <?=$_SESSION["travel_step"]["1"]["job"]=="3"?"selected='selected'":""?>>미취학아동</option>
							<option value="4" <?=$_SESSION["travel_step"]["1"]["job"]=="4"?"selected='selected'":""?>>주부</option>
						</select>
					</div>
				</li>
				</ul>
...
$(document).on('click','a[name=btnNext]',function() {
	...
	$.ajax({
		type : "POST",
		url : "/travel/meritz/common_longterm/process_step0102.php",
