...
$sql="select * from plan_code_type_longterm where company_type=6 and title <> '' and trip_type='".$tripType."' order by no asc";
...
for ($i=0;$i<count($_SESSION["travel_step"]["1"]["member"]);$i++) {
	if ($_SESSION["travel_step"]["1"]["member"][$i]["birth"]!='' && $_SESSION["travel_step"]["1"]["member"][$i]["gender"]!='') {
		...
		if ($cal_age >= 0 && $cal_age <= 14) {
			$cal_type="1";
		} elseif ($cal_age >= 15 && $cal_age <= 60) {
			$cal_type="2";
		}else{
				$json_code = array('result'=>'false','msg'=>'장기 여행자보험은 60세이상 고객님께서는 가입하실 수 없습니다.');
...
// 고급형(3), 표준형(2), 실속형(1)
$sql="select plan_title, left(MIN(plan_type),1) as plan_type_src from plan_code_longterm where company_type=6 and 
	trip_type='".$tripType
...
for($i=0;$i<count($arrPlanList);$i++) {
	...
	$sql="select * from plan_code_longterm where company_type=6 and trip_type='".$tripType."' 
	...
	for($i_member=0;$i_member<count($arrMember);$i_member++) {
		$sql_price="select price from plan_code_price_longterm a
			left join plan_code_longterm b
			on a.plan_code = b.plan_code and a.company_type = b.company_type and a.trip_type = b.trip_type
			where a.company_type=6 and a.trip_type='".$tripType."'  and a.plan_type like '%".$arrPlanList[$i]["plan_type_src"]."%'
			and b.cal_type = '".$arrMember[$i_member]["cal_type"]."' 
...
?>
				<button class="tablinks" plan_type="<?=$arrPlanList[$i]["plan_type_src"]?>" plan_title="<?=$arrPlanList[$i]["plan_title"]?>" sum_price="<?=$arrPlanList[$i]["sum_price"]?>">
					...
					<?=$arrPlanList[$i]["plan_title"]?><br/>￦<?=number_format($arrPlanList[$i]["sum_price"])?> 
				</button>
<?php
	}
?>
...
$(document).on('click','a[name=btnNext]',function() {
	$.ajax({
		type : "POST",
		url : "/travel/meritz/common_longterm/process_step02.php",
