<?
    ...
    require_once $root_dir."/travel/meritz/include/common.php";
    require_once $root_dir."/travel/meritz/config/get_site_config_member_no.php";
    ...
    include $root_dir."/travel/meritz/bill_kcp/cfg/site_conf_inc.php";       // 환경설정 파일 include
    require	$root_dir."/travel/meritz/bill_kcp/pp_cli_hub_lib.php";              // library [수정불가]
...
<?
					...
					$sql_check="select * from hana_plan_tmp where no = '".$_POST['param_opt_1']."' and order_type='1'";
...
<?
						...
						$sql_tmp="insert into hana_plan set
									member_no='".$member_no."',
									session_key='".$row_check['session_key']."',
									order_type='1',
						...
						while($row_mem=mysql_fetch_array($result_mem)) {
							...
							$sql_mem="insert into hana_plan_member set
								...
								name='".$row_mem['name']."',
                                name_eng='".trim($row_mem['name_eng'])."',
                                name_eng_first='".trim($row_mem['name_eng_first'])."',
                                name_eng_last='".trim($row_mem['name_eng_last'])."',
							...
							if ($row_mem['main_check']=="Y") {
								$mem_update="update hana_plan set 
												join_name='".$row_mem['name'].(!empty($row_mem['name_eng'])?" / ".$row_mem['name_eng']:"")."'
											where no='".$hana_plan_no."'
											";
								mysql_query($mem_update);

                                $arrParam2 = explode('|',$_POST[ "param_opt_2"]);
                                $nation_search = $arrParam2[0];
                                $plan_title = $arrParam2[1];
...
(가입조회)
https://".$_SERVER['HTTP_HOST']."/travel/meritz/m/check/check.php

* 사고접수/계약취소/정보변경 등은 유다이렉트 콜센터 1800-9010(평일09~18시)로 연락주시기 바랍니다.
* 해외여행 중 현지에서 사고 발생 시 24시간 한국어 안내가 지원되는 메리츠화재 고객콜센터 82-2-1566-7711로 연락주시면 도움 받으실 수 있습니다.";

								sendTalk_common_notice($kakao_phone, "udirect_common_notice", $msg, "https://".$_SERVER['HTTP_HOST']."/travel/meritz/m/check/check.php");
						...
						$sql_delete="delete from hana_plan_tmp where tmp_no='".$row_check['no']."'";
...
?>
<script>
	location.href="../m/trip/complete.php?check=<?=$row_check['session_key']?>";
</script>
<?
						exit;
					}
...
<script>
	...
	location.href="../m/trip/step05.php";
</script>
<?
            exit;
        }
